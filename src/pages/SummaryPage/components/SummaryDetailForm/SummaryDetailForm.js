...
export default function SummaryDetailForm(props) {
  ...
  const getAddress = () => {
    ...
    const { address1, city, postal_code: postalCode, province, country } = address;
    return `${address1} ${city} ${province} ${postalCode} ${country}`;
  };
  ...
  return (
    <div className={cx({ [styles.collapseContainer]: index !== 0 })}>
      ...
      >
        <div className={styles.collapseMain}>
          <div className={styles.confirmationRow}>
            <div className={styles.confirmation}>Confirmation #: {confirmation}</div>
          ...
          <div className={styles.otherRow}>
            ...
            <div className={styles.rightText}>${price?.amount || 0}</div>
          </div>
          {/* {
      ...
      <Modal
        title={<div className={styles.modalHeader}>Cancel This Item?</div>}
        visible={visible}
        onOk={handleOk}
      ...
      >
        ...
        <p className={styles.modalText}>
          {`You will be refunded $${(price?.amount || 0) - penalty} for this canceled item.`}
        </p>
      </Modal>
    </div>
  );
