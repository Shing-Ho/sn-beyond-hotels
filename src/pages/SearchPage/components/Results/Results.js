...
import { getFilteredHotels } from 'store/hotel/selectors';
...
const Results = ({ toggleDrawer, currency }) => {
  ...
  useEffect(() => {
    if (hotels && hotels.length > 0) {
      const geolocation = hotels[0].hotel_details.geolocation;
      setDefaultCenter([geolocation.latitude, geolocation.longitude]);
    }
    const coords = hotels.map(hotel => hotel.hotel_details.geolocation);
    setMapCoords(coords);
  }, [hotels]);
